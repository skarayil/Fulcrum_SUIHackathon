...
import { PACKAGE_ID, CONTESTANT_REGISTRY_ID } from "../config/constants";
...
export function NewSponsorDashboard() {
  ...
  // Query competition data to calculate prize per winner
  const { data: competitionDataForPrize } = useSuiClientQuery(
    "getObject",
    {
      id: distributionCompetitionId,
      options: {
        showContent: true,
      },
    },
    {
      enabled: !!distributionCompetitionId && distributionCompetitionId.startsWith("0x"),
    }
  );
  ...
  const handleCreateCompetition = async () => {
    ...
    try {
      ...
      const [coin] = txb.splitCoins(txb.gas, [prizeAmountNum]);
      const rulesBytes = Array.from(new TextEncoder().encode(rules));
      txb.moveCall({
        target: `${PACKAGE_ID}::competition::create_competition`,
        arguments: [txb.object(sponsorCapId), txb.pure.vector("u8", rulesBytes), coin],
      });
  ...
  const handleAutoGenerateTeams = () => {
    ...
    if (allAddresses.length === 0) {
      alert("No contestants available. Please add contestants first.");
      return;
    }
    ...
    alert(`✅ Generated ${teamCount} team(s) from ${allAddresses.length} contestants!`);
  };
              ...
              <Tabs.Content value="competition">
                <Card style={styles.glassCard}>
                  <Flex direction="column" gap="5">
                    ...
                    <Flex direction="column" gap="3">
                        <Text size="2" weight="bold" style={{ color: "#60a5fa" }}>Prize Amount (MIST)</Text>
                        <TextField.Root 
                          type="number" 
                        ...
                        <Text size="1" color="gray">Current: <span style={{ color: "#eab308" }}>{(Number(prizeAmount) / 1_000_000_000).toFixed(2)} SUI</span></Text>
                    </Flex>
